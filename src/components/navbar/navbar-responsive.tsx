...
export default function NavigationResponsive() {
  ...
  return (
    <nav className="w-full h-20 border-radius-md shadow-md fixed top-0 z-10 bg-white flex items-center">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Sheet>
            ...
            <SheetContent className="p-2 space-y-5">
              ...
              <SheetClose
                asChild
                className="absolute h-10 w-10 right-4 top-4 bg-white"
              ></SheetClose>
              ...
              {NavBarStructure.map((item) => (
                <div key={item.id} className="flex justify-end">
                  <a
                    href={item.link}
                    className="w-full text-center font-semibold text-black uppercase"
                  >
                    {item.name}
                  </a>
                </div>
